...
uint64_t number_of_seconds_to_hammer =18000;
  ...
  while (1) {
    ...
    for (uint8_t* first_row_page : pages_per_row[row_index])
    {
      ...
      for (uint8_t* second_row_page : pages_per_row[row_index+2])
      {
        ...
        if (get_dram_mapping((void *)get_physical_addr((uint64_t)first_row_page)) != get_dram_mapping((void *)((uint64_t)second_row_page)))
        {
        ...
        for (const uint8_t* target_page8 : pages_per_row[row_index+offset]) {
          ...
          flush(& target_page[index]);
            if (target_page[index] != VAL) {
              ...
              fprintf(fp,"%lx,%lx,%lx,0x%016lx\n",myflip, GetPageFrameNumber(pagemap,first_row_page)*0x1000,GetPageFrameNumber(pagemap,second_row_page)*0x1000,target_page[index]-VAL);
              printf("%lx,0x%016lx\n",myflip, target_page[index]-VAL);
