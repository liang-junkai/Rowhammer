...
int main(int argc, char *argv[]) {
    ...
    uintptr_t attackPages[NUMBER_PAGES] = { 0x1dee24, 0X1cd234};
    uintptr_t abovePages[NUMBER_PAGES] = { 0x1dee4b, 0x1cd25b};
    uintptr_t belowPages[NUMBER_PAGES] = { 0x1dee03, 0x1cd213};
   ...
   // sleep(5);
    ...
    check(pages);
    for(int i = 0; i < NUMBER_PAGES; i++) {
            memset((char *) pages[i].pageVA, 0xFF, PAGE_SIZE);
        }
    printf("wait for hammering\n");
    sleep(5);
    FILE *fp=fopen("values","w");
    for(int i = 0; i < NUMBER_PAGES; i++) 
    {
    printf("Checking Page: %lx\n",pages[i].pageNumber);
    for(int j = 0; j < 512; j++)
    {
            fprintf(fp,"addr:%lx, %lx\n", pages[i].pageNumber*0x1000 + j*8, * ((int *) (pages[i].pageVA + j)));
    }
    }
