...
//
void flush(const void *p) { asm volatile("clflush 0(%0)\n" : : "c"(p) : "rax"); }
...
ssize_t OFFSET2 = -1;
uint64_t myflip, count10=0;
FILE *fp=fopen("./bitflip_addrs_full","a+");
...
//  static const size_t h2[] = { 16, 20 };
 ...
 // static const size_t h5[] = { 6 };
  static const size_t h0[] = {  17, 21 };
  static const size_t h1[] = { 16,20 };
  static const size_t h2[] = { 15,19 };
  static const size_t h3[] = { 14, 18};
  static const size_t h4[] = { 6, 13 };
  ...
  //for(row_index=42100;row_index<60000;row_index+=7) {
    ...
    for (uint8_t* first_row_page : pages_per_row[row_index])
    {
      ...
      for (uint8_t* second_row_page : pages_per_row[row_index+2])
      {
        ...
        if (get_dram_mapping((void *)get_physical_addr((uint64_t)first_row_page)) != get_dram_mapping((void *)(get_physical_addr((uint64_t)second_row_page))))
        {
...
#define VAL ((uint64_t)(0))
#define VAL2 ((uint64_t)(0xFFFFFFFFFFFFFFFF))
        ...
        for (const uint8_t* target_page8 : pages_per_row[row_index+offset]) {
          ...
          flush(& target_page[index]);
            if (target_page[index] != VAL) {
              ...
              myflip=GetPageFrameNumber(pagemap, (uint8_t*)(target_page + index))*0x1000+(((size_t)(target_page + index))%0x1000);
              fprintf(fp,"%lx,%lx,%lx,%d,%d\n", GetPageFrameNumber(pagemap,first_row_page)*0x1000,GetPageFrameNumber(pagemap,second_row_page)*0x1000,myflip,ffsll(target_page[index]-VAL)-1,1);
              //printf("%lx,%d\n",myflip, ffsll(target_page[index]-VAL));
        ...
        for (const uint8_t* target_page8 : pages_per_row[row_index+offset]) {
          ...
          for (uint32_t index = 0; index < 512; ++index) {
          flush(& target_page[index]);
            if (target_page[index] != VAL2) {
              ++number_of_bitflips_in_target;
              //fprintf(stderr,"%lx,%lx,%lx\n",(uint8_t*)(target_page),index,(uint8_t*)(target_page + index));

              printf("[!] Found %zu. flip (0x%016lx != 0x%016lx) in row %ld (%lx) (when hammering "
                  "rows %zu and %zu at first offset %zu and second offset %zu\n", number_of_bitflips_in_target, target_page[index], VAL2, row_index+offset,
                  GetPageFrameNumber(pagemap, (uint8_t*)(target_page + index))*0x1000+(((size_t)(target_page + index))%0x1000),
                  row_index,row_index +2,OFFSET1 < 0?-(OFFSET1+1):OFFSET1,OFFSET2 < 0?-(OFFSET2+1):OFFSET2);

              myflip=GetPageFrameNumber(pagemap, (uint8_t*)(target_page + index))*0x1000+(((size_t)(target_page + index))%0x1000);
              fprintf(fp,"%lx,%lx,%lx,%d,%d\n", GetPageFrameNumber(pagemap,first_row_page)*0x1000,GetPageFrameNumber(pagemap,second_row_page)*0x1000,myflip,ffsll(VAL2-target_page[index])-1,0);
              //printf("%lx,%d\n",myflip, ffsll(target_page[index]-VAL));
